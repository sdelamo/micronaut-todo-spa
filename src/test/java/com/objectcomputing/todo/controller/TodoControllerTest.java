...
import com.objectcomputing.todo.utils.TodoJdbcCrudRepository;
...
class TodoControllerTest implements PostgresTestPropertyProvider {
    ...
    void getTodoRetrievesEveryTodoOfAUser() {
        ...
        List<Todo> todos = client.retrieve(HttpRequest.GET("/api/v1/todo"), Argument.listOf(Todo.class));
        assertNotNull(todos);
        assertEquals(0, todos.size());
        HttpResponse<?> response = client.exchange(HttpRequest.POST("/api/v1/todo", Collections.singletonMap("action", "setup jacoco")));
        assertEquals(HttpStatus.CREATED, response.getStatus());
        todos = client.retrieve(HttpRequest.GET("/api/v1/todo"), Argument.listOf(Todo.class));
        assertNotNull(todos);
